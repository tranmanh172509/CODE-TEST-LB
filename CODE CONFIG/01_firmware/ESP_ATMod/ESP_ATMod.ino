...
BearSSL::X509List    *clientCert;  // public keyfor client validation
BearSSL::PrivateKey  *clientKey;   // private key for client validation
...
uint8_t gsCipSslAuth = 2;     // command AT+CIPSSLAUTH: 0 = none, 1 = fingerprint, 2 = certificate chain
...
/*
   Local prototypes
*/
//static void loadCertificateFromFile();
static void loadCertificateFromVariable();
...
void setup()
{
  ...
  //  Serial.begin(9600);
  Serial.begin(57600);     // default baudrate to work with "LbSPISerial"
  ...
  // loadCertificateFromFile();  // Load certificates from LittleFS

  loadCertificateFromVariable();  // Load certificates from variables
...
void loop()
{
  ...
  if (Serial.availableForWrite())
  {
    ...
    for (uint8_t i = 0; i < SERVERS_COUNT; ++i)
    {
      ...
      //WiFiClient cli = servers[i].available();  // deprecated: Renamed to accept()
...
/*---------------------------------------------------------------------*/
static void loadCertificateFromVariable() {
  CAcert = new BearSSL::X509List(serverCA_content);
  Serial.println(F("Loaded CAcert."));

  clientKey  = new BearSSL::PrivateKey(clientKey_content);
  clientCert = new BearSSL::X509List(clientCert_content);
  Serial.println(F("Loaded clientKey & clientCert."));
}
