...
//      else if (gsCipSslAuth == 2)
        {
          // associate server certificate
          static_cast<BearSSL::WiFiClientSecure *>(cli)->setTrustAnchors(CAcert);
          // associate client certificate & private key
          static_cast<BearSSL::WiFiClientSecure *>(cli)->setClientRSACert(clientCert, clientKey);
          Serial.println(F("certificate chain verification"));
        }
        else
        {
          delete cli;
